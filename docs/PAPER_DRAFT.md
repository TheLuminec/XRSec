...
**Status: complete first draft, under review (2026-09-17).** Every number is taken from
`docs/PROGRESS_REPORT.md` and the certificates it cites; where two internal sources disagreed, the later
corrected value was used. Level intervals for the zero-shot and exposed arms are the
`docs/acceptance/schach_paired.json` bootstrap (a separate draw from `across_xr_alignment_aggregate.json`,
differing in the third decimal). The three-seed cross-arm contrasts of Section 6.5, the ten-minute
`raw`/`dyn` intervals of Section 6.6 and the sample standard deviations of Section 6.9 come from
`docs/acceptance/schach_paired_review_addenda.json`, computed from the committed per-user arrays and
gated against the certificates they derive from.
...
Motion-based identification in extended reality is near-solved within an application and collapses
across applications: the reference study reports 83.1% rank-1 identification within an application
and 18.0% across applications among seventeen unseen users (chance 5.88%), using head motion and both
hand controllers. We re-assess that cross-application risk on the same corpus, user split and metric
using **head motion only**, so that the method applies to AR glasses, which have no hand controllers.
Every contrast is against the reference study's *released model*, paired user by user with a cluster
bootstrap over the seventeen test users. A head-only model trained jointly on 3,072 identities from
other corpora and the reference study's own twenty-three training users — far more identities than
theirs — reaches 0.299 cross-application rank-1 at N=17 under their metric against their 0.180, a
paired difference of **+0.119 [+0.050, +0.192]**, better on fifteen of seventeen users; under our
template metric the contrast is **+0.176 [+0.092, +0.260]**. A model that never sees the corpus is
**unresolved** against the reference (+0.025 [−0.031, +0.080]) and is not reported as a beat. With
one application held out of training entirely, exposure to the other four still carries +0.049
[+0.021, +0.078] into it, even for applications absent from every pretraining corpus. Three further
results qualify the risk picture. Keeping absolute head pose reaches 0.351 cross-application **after
a single training epoch**, +0.117 [+0.042, +0.192] above our behaviour-only figure, so a
behaviour-only assessment understates the risk. The orthogonal embedding alignment the reference
study proposes as future work does not carry when fitted on training users (+0.011 [−0.020, +0.041]
against a registered band of +0.05 to +0.20): the map helps, if at all, only the people it was fitted
on, and only thirty-two participants outside the evaluation split exist to fit it on. And
cross-application rank-1 varies across individuals far more than its mean suggests: in the reference
study's own per-user values it runs from 0.068 to 0.371 on a single window and from 0.043 to 0.818
over ten minutes. Predictions were registered with falsifiers before the runs, and negative outcomes
are reported alongside positive ones.
...
the number against which everything here is measured. Their headline figure of 78.5% is a third
quantity: it is computed with a reference set drawn from all five applications, including the query's
own, so it is neither a within- nor a cross-application figure, and it inherits the overlap property
discussed in Section 5.4. We do not compare against it.
...
10 s against 15 s — but also with far more training identities, 3,095 against their 23, all but those
23 drawn from corpora they did not use. We state both asymmetries where we win and borrow neither as an
excuse where we do not.
...
1. **A head-only beat of the reference study's released model on its own corpus, people and metric,
   under a sensor handicap and with a training-identity advantage.** Trained jointly on other corpora
   and the reference study's own twenty-three training users (3,095 identities against their 23),
   +0.119 [+0.050, +0.192] under their metric and +0.176 [+0.092, +0.260]
...
4. **A negative answer, with a mechanism, to the reference study's own proposed fix.** Train-user-only
   ...
   and the corpus offers at most thirty-two participants outside the evaluation split (users 0–31) to
   fit on, a number no amount of pretraining raises (Section 6.7).
...
current generation of hardware: Rogers et al. (2015) identified wearers of a head-mounted device from
head motion alone, Miller, Herrera, Jun, Landay and Bailenson (2020) showed that head and hand
trajectories identify viewers of 360-degree video, and a separate group, Miller, Banerjee and Banerjee
(2021), matched users across VR systems with a Siamese network. Rack, Fernando, Yalcin, Hotho and
Latoschik (2023) contributed both a dataset and a protocol:
...
metric-learning objectives is what makes generalisation to unseen users measurable at all. Rack, Kobs,
Fernando, Hotho and Latoschik (2023) train a similarity model on a large motion corpus and transfer it to new populations, which is
...
**Cross-application identification.** The reference study for this paper, Schach et al. (2026), is the
first to evaluate identification across five applications for the same population, with a disjoint user
split, and it is the work we compare against throughout. The one other cross-application result we know
of is Baldoni et al. (2025), on the Questset corpus: above 95% within-game identification with a
classification model and no model above 0.30 cross-game at a gallery of thirty, across the two
applications each participant played. That figure is not a competitor to ours and is not treated as one:
...
application changes. Each session is nominally ten to fifteen minutes; in the raw release the 245
cells hold 40,629 to 206,181 frames (median 89,580), about seven to thirty-seven minutes at the
measured rate, and the longest cell comprises two recordings of one game under a single identifier
rather than one unbroken session. Recording is at about 92 Hz (the median over cells) on one headset
model, an HP Reverb G2, in a ten-square-metre laboratory area. The files carry positions in centimetres
and a y-up convention that matches our pipeline, so no axis remapping is required. One conversion
defect is on record: for
that two-recording cell (a training-split participant, user 1, in Superhot VR) our converter kept only
the second recording, 124,221 of 206,181 frames; the other 244 cells match the raw release exactly, and
no figure on the test users is affected.
...
| validation | 23–31 | 9 | epoch selection for the corpus-only arms and, beside a pretraining draw, for the large exposed arm; dropped entirely by the matched and leave-one-application-out arms; with the training users, the alignment fitting population (0–31) |
...
plus breaks, in the same fixed order for every participant — Synth Riders, Superhot VR, Beat Saber,
Half-Life: Alyx, then the Social VR scenario — so a cross-application pair carries real temporal
separation of up to roughly an hour, but never a different day. (The numeric application identifier in
the files is not the play order.) Three consequences follow.
...
recordings, which is the structural point we return to in Section 5.4. Finally, because the order is
fixed, application is confounded with position in the sitting — warm-up, familiarity, fatigue — for
every participant alike, and nothing in this corpus can separate the two.
...
**who-is-alyx** (Rack, Fernando, Yalcin, Hotho and Latoschik, 2023) supplies 76 players of Half-Life: Alyx across 146 sessions, mostly
...
attribution travels with our converted copy. BOXRR-23 is listed under CC BY-NC-SA 4.0 by its distributor, but our use of it is governed by a
signed Data Use Agreement with the University of California, Berkeley, with institutional ethics approval in place in advance of use
...
classifier head is discarded after training and embeddings are compared by cosine similarity (AM-Softmax;
Wang et al., 2018). Training uses per-dataset normalisation with statistics fitted on training users
only, 120 epochs and a patience of fifteen. The epoch is selected on Across-XR users 23–31 for the
corpus-only arms; on a held-out quarter of the pretraining identities together with users 23–31 for the
large exposed arm; and on that quarter alone — pinned to an explicit list so that training sets could be
matched after the draw — for the matched pair and the leave-one-application-out arms, which drop users
23–31 entirely (neither trained on, validated on nor evaluated). The verification score used only as a
gate referent (Section 5.5) draws its positive pairs across sessions and its negative pairs within a
dataset.
...
The same applies to the figure their paper reports as its overall similarity-model accuracy, **78.5%**,
...
Predictions P1–P3 were registered with bands or directions, and falsifiers, dated, before any
cross-application run; arm-level bands and eight subsequent amendments are recorded in the same way. Amendments are kept in
...
band (the registration named no outcome for 0.12 to 0.18, a gap that did not bite here and that we
record rather than close after the fact). Scored through the reference study's own calculator
(direction D1) the same arm reads
...
head **plus both hand controllers** and 15 s windows against our 10 s — and, in the other direction,
with 3,095 training identities against their 23. The paired contrast was registered before it was
computed, at +0.08 to +0.20 under their metric and +0.05 to +0.17 under ours, but with the exposed
arm's own level under our metric (0.375) and their published 0.180 already in hand: what the
registration predicted was the metric translation — our figure under their decision rule and theirs
under ours — not the arm. The first landed inside its band and the second landed 0.006 past its upper
edge, which we declare rather than argue either way. Their side of every contrast is a single training
run; ours averages three seeds inside each user before the bootstrap (per-seed range 0.010), which the
bootstrap over users accommodates but which is stated here rather than left implicit.
...
**Against our own zero-shot arm, exposure is worth +0.141 [+0.101, +0.182]** on a single window,
paired per user over three seeds of each arm. This is the cleanest statement of what exposure buys,
because the two arms share every training window except the 20,896 from Across-XR users 0–22, and it
was registered before the arm ran with +0.05 as the level above which a dose under four percent would
count as carrying; it clears that level with its whole interval. It is also why the identity-count
result of Section 6.5 has to be read jointly with exposure rather than on its own.
...
Among the twenty ordered application pairs, scored under the reference study's metric, the exposed arm
beats the reference study **resolvably in eleven and loses none**; the zero-shot arm beats it resolvably
in three and loses none — which is precisely why its overall contrast remains unresolved, and the three
should not be quoted as zero-shot beating the reference study. The two largest paired gains in both of
our arms are the rhythm-game pair in both directions. Under our metric those two cells read: Synth
Riders → Beat Saber 0.475 [0.401, 0.547] zero-shot and 0.591 [0.536, 0.654] exposed; Beat Saber →
Synth Riders 0.432 [0.344, 0.522] and 0.546 [0.472, 0.620]. That affinity
...
**Three further registered predictions on this design: one held, two failed.** The per-application
ordering was registered as the mechanism claim — the rhythm games should carry best and the Social VR
scenario least. The first half held: Beat Saber (+0.084) and Synth Riders (+0.065) are the two largest
carries. The second half failed: Half-Life: Alyx is the smallest at +0.026 and Social VR sits mid-table
at +0.038, so the task-structure ordering holds at the top and not at the bottom. A third registration,
that the test-fitted alignment ceiling of Section 6.7 would stay below +0.03 on these embeddings, failed
on three of five: it reads +0.003, +0.039, +0.135, +0.136 and +0.131 with Superhot VR, Half-Life: Alyx,
Beat Saber, Synth Riders and Social VR held out respectively. Those five single-run readings are further
evidence of the run-dependence Section 6.7 reports, and the honest train-user fit is resolvably above
zero on none of them.
...
3,072 to 495 trained identities changes cross-application rank-1 by −0.016 [−0.040, +0.007]. **With**
exposure it is not flat: the 495-identity exposed arm loses to the 3,095-identity exposed arm by −0.068
[−0.099, −0.037], and the 495-identity exposed arm gains +0.089 [+0.047, +0.131] over its own unexposed
control. That last contrast carries the band the original exposure registration attached to it, +0.05
to +0.20 with a falsifier below +0.03: the mean sits inside the band and the interval's lower edge falls
0.003 below it, on one seed, which we declare rather than argue. Identity count and exposure are
therefore not two independent additive levers; the second is what makes the first pay across an
application boundary. (Every cross-arm contrast in this section pairs a single-seed arm against the
three-seed arm's per-user seed mean, as Section 5.3 describes; the aggregate certificate's seed-1
pairings, −0.013 and −0.061, agree in sign and verdict.)

**Dose cannot account for the pair, and correcting for it widens the effect rather than narrowing it.**
Halving the in-domain windows at fixed identities costs −0.028 [−0.062, +0.009], and a 20% cut — the
non-held-out cells of the leave-one-application-out arms of Section 6.4 against the fully exposed arm at
the same identity count — costs −0.009 [−0.025, +0.008]. Both intervals span zero, so dose is at most a
small term in this range — and the arm that *loses* by 0.068 is the one with the **higher** dose (14.1%
against 3.87%).
...
[+0.042, +0.192]**. This contrast was registered with a band of +0.00 to +0.06 and a falsifier below
−0.03. The falsifier is excluded by 0.07; the mean sits at twice the band's upper edge; and the
interval's lower bound (+0.042) falls inside the band, so by our own rule the effect is decisively
positive and its *size* is unresolved between "inside" and "above". The registration named no outcome
for landing above the band — the partition defect Section 5.5 describes — and we record it as one rather
than widen the band afterwards. Within an application the same substitution is worth +0.223 [+0.184,
+0.263] (0.723 against 0.500), which is larger, as the registration predicted it would be, and which is
what carries the within-application placement cue this corpus has (lateral P(within<between) 0.7525
within an application against 0.527 across applications). The within-application `raw` cell is
therefore not quoted as a biometric figure.

**The finding is not the size of the gain; it is the epoch at which it appears.** Every `raw` seed
selected **epoch 1**: under a 120-epoch budget with a patience of fifteen, training stopped at epoch 16
and the validation-selected checkpoint was the first in every seed, and the exposed `raw` arm did the
same. A model one epoch from initialisation — about five hundred gradient steps at batch 1,024 over
519,211 windows, where every behavioural arm selects an epoch above one hundred — reaches 0.351
cross-application from the
head alone. The cue is learned almost at once, which is worse news than a trained model reaching the
same place. Two comparators bound what it is. The `raw` arm's verification score against a
training-free recorded-position lookup (0.704–0.733 against 0.585–0.598) says it is reading more than
mean position alone: posture as well as height. The training-free comparator on the identification
metric itself — the mean-position lookup at N=17 that Section 6.6.1 runs on Questset — has not been run
on the Across-XR test users, so how much of 0.351 needs no model at all is not measured here and is
listed in Section 8 as outstanding.
...
reads 0.404 against the exposed `dyn` arm's 0.375, a difference of +0.029 [−0.068, +0.134] on one `raw`
seed against three `dyn` seeds, spanning both edges of the same +0.00 to +0.06 band; within an
application the exposed pair reads 0.730 against 0.616, +0.114 [+0.055, +0.175]. Over ten minutes the
behavioural model leads outright: 0.711 against 0.497, a paired −0.214 [−0.337, −0.085] for `raw` (seed
against seed, −0.196 [−0.329, −0.062]). Without exposure the ten-minute contrast is unresolved — `raw`
0.434 against `dyn` 0.357, +0.077 [−0.055, +0.208] — so the reversal is a property of the exposed arm.
Averaging more evidence lifts a learned per-window signal, whose error is variance, and cannot lift a
static cue, whose error is a per-recording bias.
...
application, and cross-application with exposure, with per-seed dots and the "epoch 1 selected;
patience stopped training at 16" annotation. Chart spec C7.]`
...
### 6.7 The reference study's proposed fix does not carry, and the fit is not why
...
+0.05 to +0.20 is excluded. The test-fitted ceiling on our own zero-shot embedding — their illegitimate
route, reproduced — is +0.026 [+0.000, +0.051], **thirteen times smaller** than the +0.34 they measure,
and it is itself run-dependent: at identical configuration on the exposed arm, three seeds read +0.148,
−0.004 and +0.001, and the five leave-one-application-out checkpoints of Section 6.4 read +0.003 to
+0.136. A single-run diagnostic bound of that kind is not evidence that application embeddings
...
**The mechanism is the fitting population, and it is the actionable half.** The permuted-correspondence
null *hurts* by −0.074, so the orthogonal component our fit recovers is real and person-specific; it is
simply tiny. The honest fit fails not because the fit is rank-deficient but because of the
**test-fitted advantage**: the ceiling variant fits on the seventeen people it is then scored on and
reaches +0.148, while the honest variant fits on thirty-two *other* people and reaches −0.008 — more
correspondences did not help, because the correspondences are not the same people. (The rank argument
is real and belongs to a different variant: the unrestricted 128-d fit, which must invent an arbitrary
96-dimensional complement from at most thirty-two correspondences, loses −0.055 [−0.086, −0.029] on the
zero-shot arm and is negative at the mean on all nine checkpoints on which it was run. It is not the
reason the honest route fails, and conflating the two attaches a wrong mechanism to a right conclusion.)

**What follows for the field is a test, not yet a specification.** The correspondences available for an
honest fit are bounded by the number of participants outside the evaluation split — thirty-two here,
the same thirty-two the reference study's own protocol allows — and no amount of pretraining raises that
number. Whether *more* such participants would make the fit transfer is a hypothesis this corpus cannot
settle, and the mechanism above cuts both ways: a map fitted on thirty-two people did not transfer to
seventeen others, which is consistent both with too few people to average out person-specific structure
and with structure that is person-specific and never averages out. The test that separates them is
cheap and runs on embeddings that already exist: fit on k of the thirty-two for k = 8, 16, 24 and 32,
and read whether the honest gain rises with k. If it rises, a corpus with many more multi-application
participants is the specification a future collection should be designed against; if it is flat,
alignment is closed for this class of embedding regardless of corpus size. The honest answer to their
future-work proposal is therefore not "we failed to make it work" but **"this corpus cannot support it,
and here is the measurement that decides whether any corpus could"**.
...
It lifted them by **+0.018 only** (0.180 → 0.199), missing the band's lower edge by 0.001. We do not
argue the edge, in the same way we do not argue a 0.001 edge that falls in our favour. The useful half
is the mechanism: template averaging buys our embeddings +0.028 (zero-shot) and +0.075 (exposed) and
...
pretraining corpus improves cross-domain transfer, by swapping fifty BOXRR-23 Beat Saber identities for
fifty identities from Nymeria (Ma et al., 2024) — daily-life activity recorded on AR glasses — at a
fixed identity count of 419, measured as pooled verification transfer to seven held-out seated corpora
under `dyn` over five paired seeds. It measured −0.0012 [−0.0045, +0.0020]
...
The same holds of our own arms. Per-user spread under their metric (sample standard deviation across the
seventeen) is 0.093 for the reference model, 0.071 for our zero-shot arm and 0.113 for our exposed arm,
over per-user ranges of 0.068–0.371, 0.105–0.363 and 0.148–0.528 respectively.
...
−0.016 [−0.040, +0.007] without exposure — and greater *activity* diversity in the pretraining corpus did
...
**Alignment as a corpus question.** The most useful output of our negative alignment result is not the
null itself but the number thirty-two and the test it sets up. Fitting a transformation between two
applications' embedding spaces requires people recorded in both who lie outside the evaluation split,
and the fully crossed corpus that makes the whole cross-application literature possible offers
thirty-two of them. Whether more would help is not established — the map fitted on those thirty-two did
not transfer to seventeen others, which a larger fitting population might or might not cure — but the
subsampling test of Section 6.7 decides it on embeddings that already exist, and its answer, not the
null, is what a future collection should be designed against. It is available now because the route was
run honestly rather than assumed to fail.
...
**A fixed play order.** Every participant played the five applications in the same sequence, so
application is confounded with position in the sitting. The per-pair figures of Section 6.3 therefore
mix an application effect with an order effect that this corpus cannot separate; a counterbalanced
collection would.
...
**Several arms are single-seed** — the two 495-identity arms (Z-676 and C2-hi), the dose arms, the
corpus-only arms, three of the five leave-one-application-out arms, the exposed `raw` arm and the margin
screen — and seed variance is arm-specific rather than a pipeline constant: trained-out arms show a seed range of 0.010 on the primary
metric while the epoch-1 `raw` arm shows 0.029.

**The headline pairs three seeds of ours against one training run of theirs.** Their released model is
a single checkpoint; our exposed arm is averaged over three seeds inside each user before the bootstrap.
That is valid for a user-level interval, but the two sides carry different run-to-run variance, and only
ours is measured: a per-seed range of 0.010 on the primary metric.

**The training-free static comparator was not run as rank-1 on Across-XR.** The epoch-1 `raw` result of
Section 6.6 is set against a recorded-position lookup as a verification score on the seventeen test
users, not as a rank-1 identification figure; the rank-1 form of that lookup was run on Questset only.
...
targeted the zero-shot arm; the exposed arm's paired contrast against the reference study was registered
before that comparison was computed, but after the arm's own level was known, and the decision to lead
with it was made afterwards. We say so rather than presenting the registration as covering more than it
does.
...
this particular system", and we do not have one. The per-user rank disagreement reported in Section 6.9 is
...
person-specific: it helps, when it helps at all, only the people it was fitted on, and the corpus
offers at most thirty-two participants outside the evaluation split (users 0–31) to fit it on. Whether a
far larger fitting population would recover a transformation that does transfer is untested here, and it
is the most actionable question this work leaves: the corpus that could answer it is one in which many
people are recorded in many applications.
...
1. L. Schach, C. Rack, R. P. McMahan, M. E. Latoschik. *Motion-Based User Identification across XR and
   ...
   7:1743491, 2026 (published 6 March 2026). doi:10.3389/frvir.2026.1743491. Preprint arXiv:2509.08539
   (September 2025). Dataset (Across-XR), CC BY-NC-SA
...
5. C. Rack, K. Kobs, T. Fernando, A. Hotho, M. E. Latoschik. *Versatile User Identification in Extended
   Reality using Pretrained Similarity-Learning.* arXiv:2302.07517, 2023 (v2, April 2024; no journal
   version located).
6. C. Rack, A. Hotho, M. E. Latoschik. *Comparison of Data Encodings and Machine Learning Architectures
   for User Identification on Arbitrary Motion Sequences.* IEEE International Conference on Artificial
   Intelligence and Virtual Reality (AIVR), 2022. *(The SR/BR/BRV/BRA encodings.)* The encodings are
   implemented in C. Rack, L. Schach, M. E. Latoschik, *Motion Learning Toolbox* (software, CC BY-NC-SA
   4.0), github.com/cschell/Motion-Learning-Toolbox, commit b8189e6c; no separate paper describes the
   toolbox.
7. S. Baldoni, F. Battisti, F. Chiariotti, F. Mistrorigo, A. B. Shofi, P. Testolina, A. Traspadini,
   A. Zanella, M. Zorzi. *Questset: A VR Dataset for Network and Quality of Experience Studies.* ACM
   Multimedia Systems Conference (MMSys), 2024. doi:10.1145/3625468.3652187. Data (CC BY 4.0):
   researchdata.cab.unipd.it/1239/ (version of 20 March 2024 of record 1179;
   doi:10.25430/researchdata.cab.unipd.it.00001179). **[Author order taken from the data record; confirm
   against the ACM page before submission.]**
8. S. Baldoni, S. Benhamadi, F. Chiariotti, M. Zorzi, F. Battisti. *Movement- and Traffic-based User
   Identification in Commercial Virtual Reality Applications: Threats and Opportunities.* IEEE Conference
   on Virtual Reality and 3D User Interfaces (VR), pp. 72–81, 2025. arXiv:2501.16326. *(On Questset; test
   users are seen during training, and it uses the head plus both controllers, so it measures a different
   quantity from this work.)*
9. M. R. Miller, F. Herrera, H. Jun, J. A. Landay, J. N. Bailenson. *Personal identifiability of user
   tracking data during observation of 360-degree VR video.* Scientific Reports 10, 17404, 2020.
   doi:10.1038/s41598-020-74486-y.
10. R. Miller, N. K. Banerjee, S. Banerjee. *Using Siamese Neural Networks to Perform Cross-System
    Behavioral Authentication in Virtual Reality.* IEEE Virtual Reality and 3D User Interfaces (VR),
    pp. 140–149, 2021.
11. C. E. Rogers, A. W. Witt, A. D. Solomon, K. K. Venkatasubramanian. *An approach for user identification
    for head-mounted displays.* ACM International Symposium on Wearable Computers (ISWC), pp. 143–146,
    2015.
12. F. Wang, J. Cheng, W. Liu, H. Liu. *Additive Margin Softmax for Face Verification.* IEEE Signal
    Processing Letters, 25(7):926–930, 2018.
13. L. Ma, Y. Ye, F. Hong, et al. *Nymeria: A Massive Collection of Multimodal Egocentric Daily Motion in
